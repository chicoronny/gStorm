...
import org.gstorm.gui.FastMedianPanel;
...
import org.gstorm.math.QuickSelect;
...
public class NMSFastMedian<T extends RealType<T> & NativeType<T>> extends SingleRunModule implements PreProcessor<T> {
		...
		if (counter % nFrames == 0) {// make a new list for each Callable
			...
			if (interpolating) {
				...
				for (int i = 0; i < nFrames; i++) {
					final Frame<T> filtered = Utils.substract(frameB, transferList.poll());
	...
	private void interpolate(Queue<Frame<T>> transferList) {
		...
		for (int i = 0; i < nFrames; i++) {
			...
			final Frame<T> filtered = Utils.substract(
	...
	private void lastFrames(Queue<Frame<T>> transferList) {
		...
		// create last frame
		Frame<T> lastFrame = Utils.substract(frameB,transferList.poll() );
	...
	public FrameElements<T> detect(Frame<T> frame) {
		...
		final T max = Utils.computeMax(Views.iterable(interval));
