...
class BSplines {
	...
	void saveAsCSV(String path){
		...
		try {
			...
			w.write(Utils.doubleArrayToString(knotsX)+"\n");
			for (PolynomialFunction aPolynomsX : polynomsX)
				w.write(Utils.doubleArrayToString(aPolynomsX.getCoefficients()) + "\n");
			w.write("--\n");
			w.write(Utils.doubleArrayToString(knotsY)+"\n");
			for (PolynomialFunction aPolynomsY : polynomsY)
				w.write(Utils.doubleArrayToString(aPolynomsY.getCoefficients()) + "\n");
			w.write("--\n");
			w.write(Double.toString(findIntersection())+"\n");
			w.write(Double.toString(zStep)+"\n");
			w.write(Utils.doubleArrayToString(zgrid)+"\n");
			w.write(Utils.doubleArrayToString(bestE)+"\n");
			w.close();
		} catch (IOException e) {
			e.printStackTrace();
		}
