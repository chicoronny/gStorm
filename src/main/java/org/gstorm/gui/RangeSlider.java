	...
	private class RangeSliderUI extends BasicSliderUI {
		...
		protected void calculateThumbLocation() {
			...
			if (slider.getSnapToTicks()) {
				...
				int majorTickSpacings = slider.getMajorTickSpacing();
				int minorTickSpacings = slider.getMinorTickSpacing();
				int tickSpacing = 0;

				if (minorTickSpacings > 0) {
					tickSpacing = minorTickSpacings;
				} else if (majorTickSpacings > 0) {
					tickSpacing = majorTickSpacings;
				}
		...
		public void paintTrack(Graphics g) {
			...
			if (slider.getOrientation() == JSlider.HORIZONTAL) {
				// Determine position of selected range by moving from the
				// middle
