...
import org.gstorm.interfaces.PreProcessor;
import org.gstorm.interfaces.Store;
import org.gstorm.math.CentroidFitterRA;
...
import org.gstorm.modules.StoreSaver;
...
import org.gstorm.providers.ActionProvider;
import org.gstorm.providers.DetectorProvider;
import org.gstorm.providers.FitterProvider;
import org.gstorm.providers.RendererProvider;
...
public class Controller<T extends NumericType<T> & NativeType<T> & RealType<T>> extends JFrame implements ActionListener {
	...
	private void loadImages() {
		...
		if (loc_im != null) {
			cameraProperties=Utils.readCameraSettings("camera.props");
	...
	private void ppPreview() {
		...
		detResults = preprocessor.detect(Utils.substract(result,origFrame));
	...
	private void detectorPreview() {
		...
		Img<T> curImage = Utils.wrap(ip.getPixels(), new long[]{ip.getWidth(), ip.getHeight()});
		...
		final FloatPolygon points = Utils.convertToPoints(detResults.getList(), currentRoi.getBounds(), pixelSize);
		final PointRoi roi = new PointRoi(points);
		img.setRoi(roi);
	}
	...
	private void fitterPreview() { // use only CentroidFitter for performance reasons
		...
		final Img<T> curImage = Utils.wrap(ip.getPixels(), new long[]{ip.getWidth(), ip.getHeight()});
		...
		final FloatPolygon points = Utils.convertToPoints(fitResults, currentRoi.getBounds(), pixelSize);
		final PointRoi roi = new PointRoi(points);
		img.setRoi(roi);
	}
