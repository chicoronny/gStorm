...
public class CharBufferedReader extends Reader {
     ...
     * @param  in   A Reader
     */
    public CharBufferedReader(Reader in) {
    ...
    public char[] readCharLine(int lineLength) throws IOException {
    	int startChar;
    	char[] s = null;
        int i;
        int linebufLength;
            ...
            for (;;) {
                ...
                if (eol) {
                    ...
                    if (s == null) {
                    	...
                    	System.arraycopy(cb, startChar, s, linePos, linebufLength);
                    	linePos += linebufLength + 1;
                    	linebuf = Arrays.copyOfRange(s, 0, linePos);
                        //s.append(cb, startChar, i - startChar);
                        //str = s.toString();
                    }
                    nextChar++;
                ...
                if (s == null)
                    s = new char[lineLength];
                System.arraycopy(cb, startChar, s, linePos, linebufLength);
                linePos += linebufLength;
                //s.append(cb, startChar, i - startChar);         	
    ...
    public void mark(int readAheadLimit_) throws IOException {
        if (readAheadLimit_ < 0) {
            throw new IllegalArgumentException("Read-ahead limit < 0");
        }
        synchronized (lock) {
            ensureOpen();
            this.readAheadLimit = readAheadLimit_;
            markedChar = nextChar;
            markedSkipLF = skipLF;
        }
    }
